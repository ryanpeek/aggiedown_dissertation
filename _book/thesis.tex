...
\subsection{Sample collection}\label{ch1samplecollection}}
...
\hypertarget{denovo}{%
\subsection{De novo assembly}\label{denovo}}
...
data were generated from 24 \emph{R. boylii} individuals
\protect\hyperlink{supptables}{(Appendix, S2)} across coastal and Sierra
Nevada populations from California, USA. DNA was extracted with a
magnetic bead--based protocol (Ali et al.
\protect\hyperlink{ref-ali_rad_2016}{2016}) and quantified using
Quant-iT PicoGreen dsDNA Reagent (Thermo Fisher Scientific) with an
FLx800 Fluorescence Reader (BioTek Instruments). RAD libraries were
constructed using the SbfI restriction enzyme and a new RAD protocol
(Ali et al. \protect\hyperlink{ref-ali_rad_2016}{2016}). De novo loci
discovery and contig extension were carried as previously described
...
bp \href{ref:TableCh1S3}{(Appendix, S3)} which served as a de novo
partial genome reference for all subsequent downstream analyses.

\hypertarget{rapture}{%
\subsection{Rapture sequencing}\label{rapture}}

We then performed Rapture on all samples \href{ref:TableS1}{(Appendix
S1)} (2016) using 8,533 RAD capture baits (120 bp) were designed by
Arbor Biosciences from the de novo alignment (Table S4). The final
Rapture library was sequenced in 50\% of an Illumina HiSeq 3000 lane.
Rapture sequence data from each individual (Table S1) were aligned
against the de novo partial genome reference using the BWA-MEM algorithm
(Li and Durbin \protect\hyperlink{ref-li_fast_2010}{2010}) and saved to
BAM format. SAMtools was used to sort, filter for proper pairs, remove
PCR duplicates, and index binary alignment map (BAM), as well as merge
...
(minQ) of 20, and the genotype likelihood model (GL 1) (Li, 2011). To
maximize data quality, samples with less than 100,000 aligned reads were
excluded (Table S1, S2) using and only sites represented in at least
50\% of the included samples (minInd) were used. Settings used in ANGSD
for PCA to identify polymorphic sites included a SNP\_pval of 1e-6,
inferring major and minor alleles (doMajorMinor 1), estimating allele
frequencies (doMaf 2) (Kim et al., 2011), retaining SNPs with a minor
allele frequency of at least 0.05 (minMaf), genotype posterior
probabilities were calculated with a uniform prior (doPost 2), and the
doIBS 1 and doCov 1 options were used to generate PCA data. Principal
components (PC) summarizing population structure were derived from
classic eigenvalue decomposition and were visualized using the ggplot2
package in R (R Core Team
\protect\hyperlink{ref-r_core_team_r_2017}{2017}).
...
calculating a site frequency spectrum (SFS) for each population (doSaf)
(Nielsen et al. \protect\hyperlink{ref-nielsen_snp_2012}{2012}) with
...
al. \protect\hyperlink{ref-korneliussen_angsd_2014}{2014}).
...
that were estimated as described above as priors (pest) to calculate
each statistic for each site (doThetas), and then averaged to obtain a
single value for each statistic (Korneliussen et al.
...
\(\Delta \theta\) (\((\theta_\pi - \theta_S)�\)) was included to assess
the effect of genomic variation on F\textsubscript{ST} across regulation
types.
...
\caption{\label{tab:ch1TableS2}Metadata for USGS gaging stations with current and historic data available for each study river}
\centering
...
Unregulated (North Fork American). See Table \ref{tab:ch1TableS2} for
USGS gaging station information.}\label{fig:ch1Fig1map}
\end{figure}
...
approximately nine (Table 1). With genomic data, population genetic
parameters can be accurately estimated from even low sample numbers
(Hotaling et al.
...
(e.g., a \(\theta_\pi - \theta_S\) that was less than zero) (Figure 4A).
The regulated locations showed a clear trajectory of genetic diversity
loss (Figure 4A, 4B). Three of the four hydropeaking locations had the
...
trajectory at unregulated locations was largely neutral (Figure 4B).
This indicates populations in the northern Sierra Nevada which are
already limited in number are losing genetic variation, and river
regulation appears to be exacerbating these patterns. We conclude there
is evidence of recent genetic diversity loss across populations in the
regulated river systems, regardless of regulation type.
...
Sampling locations and flow characteristics. A) Map of
sampling locations spread across six rivers. B) Comparison of annual
mean daily discharge from 1981--2016 for three flow types. C) Comparison
of hourly discharge in three different flow regimes in April through
July 2012, Bypass (South Yuba), Hydropeaking (Middle Fork American), and
Unregulated (North Fork American). See Table \ref{tab:ch1TableS2} for
USGS gaging station information. Map of sampling locations in the Feather, Yuba, and
American watersheds. RABO=\emph{R. boylii}, RASI=\emph{R. sierrae}.
\begin{figure}
\includegraphics[angle=90, scale=.75]{figure/ch2/figure_01_overview_hybrid} \caption{Sampling locations and flow characteristics. A) Map of
sampling locations spread across six rivers. B) Comparison of annual
mean daily discharge from 1981--2016 for three flow types. C) Comparison
of hourly discharge in three different flow regimes in April through
July 2012, Bypass (South Yuba), Hydropeaking (Middle Fork American), and
Unregulated (North Fork American). See Table \ref{tab:ch1TableS2} for
USGS gaging station information.}\label{fig:fig1map}
\end{figure}
...
\NormalTok{ch1T1 <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(}\StringTok{"data/ch1_T1_samplesites_samples.csv"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
cols(
  ...
  regtype = col_character(),
  lat = col_double(),
  lon = col_double(),
  elev_m = col_double(),
  NHD_StO = col_integer(),
  NHD_DA_km2 = col_double(),
  n_initial = col_integer(),
  n_retained = col_integer()
)
\end{verbatim}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ch1T2 <-}\StringTok{ }\KeywordTok{read_csv}\NormalTok{(}\StringTok{"data/ch1_T2_usgs_gages.csv"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
Parsed with column specification:
cols(
  ...
  USGS_Gage_number = col_character(),
  Years_of_record = col_character(),
  Latitude = col_double(),
...
\item
  Table S3: \protect\hyperlink{rapture}{RAD Capture baits} can be found
  at
  \href{https://github.com/ryanpeek/peek_phd_supplemental_materials/blob/master/supplemental_tables/S3_capture_baits_120.fa.txt?raw=true}{github.com/peek\_phd\_supplemental\_materials}
\item
  Table S4: \protect\hyperlink{denovo}{\emph{de Novo}} alignment for
  \emph{R. boylii}. Zipped file is 4.16 MB and can be found at
  \href{https://github.com/ryanpeek/peek_phd_supplemental_materials/blob/master/supplemental_tables/S4_de_novo_assembly_300.fa.zip?raw=true}{github.com/peek\_phd\_supplemental\_materials}
\end{itemize}
...
This version of the thesis was generated on 2018-09-12 16:36:00. The
